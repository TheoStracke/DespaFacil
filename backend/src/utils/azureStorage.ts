...
function getContainerClient() {
  ...
  // Opção 1: SAS URL do container (AZURE_STORAGE_SAS_URL)
  ...
  // Opção 2: Connection string
  const connectionString = process.env.AZURE_STORAGE_CONNECTION_STRING;
  const containerName = process.env.AZURE_STORAGE_CONTAINER_NAME;
  if (!containerName) {
    throw new Error('Defina AZURE_STORAGE_SAS_URL ou AZURE_STORAGE_CONTAINER_NAME.');
  }
  if (connectionString) {
  ...
  // Opção 3: Account name + key
  const accountName = process.env.AZURE_STORAGE_ACCOUNT_NAME;
  const accountKey = process.env.AZURE_STORAGE_ACCOUNT_KEY;
  if (!accountName || !accountKey) {
    throw new Error(
      'Configure AZURE_STORAGE_SAS_URL, AZURE_STORAGE_CONNECTION_STRING ou ' +
        'AZURE_STORAGE_ACCOUNT_NAME + AZURE_STORAGE_ACCOUNT_KEY.'
    );
  }
