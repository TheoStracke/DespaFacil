...
export async function forgotPassword(email: string, captcha: string) {
  ...
  console.log('[FORGOT-PASSWORD] Bypass ativo:', bypassCaptcha);
  ...
  if (!bypassCaptcha) {
    ...
    try {
      ...
      console.error('[RECAPTCHA Enterprise] Erro:', error.message);
      throw new Error('Erro ao validar captcha: ' + (error.message || 'tente novamente'));
    }
  } else {
    console.log('[RECAPTCHA Enterprise] ⚠️ BYPASS ATIVO - Captcha não validado (apenas dev)');
  }
